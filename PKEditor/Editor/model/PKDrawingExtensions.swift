...
extension PKStroke:Copyable {
    func copy() -> PKStroke{
       
        let stroke = PKStroke(ink: self.ink, path: self.path, transform: self.transform, mask: self.mask)
        return stroke
    }
    
    func copyWith(color:UIColor) -> PKStroke{
        let ink = PKInk(.pen, color: color)
        let stroke = PKStroke(ink: ink, path: self.path, transform: self.transform, mask: self.mask)
        return stroke
    }
}
