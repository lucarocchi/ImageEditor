...
struct LayerCanvasView: UIViewRepresentable {
    ...
    init(index:Int,model:LayerModel, activeCanvasId: Binding<Int>,  sharedOffset: Binding<CGPoint>) {
        self.index = index
        self.model = model as! LayerCanvasModel
        self.model.index = index
        print("layerview canvas init")
    ...
    func makeUIView(context: Context) -> PKCanvasView {
        ...
        canvasView.minimumZoomScale = editor.minimumZoomScale
        canvasView.maximumZoomScale = editor.maximumZoomScale
        canvasView.zoomScale = editor.zoomScale
        canvasView.contentSize = editor.contentSize
        canvasView.contentOffset = editor.contentOffset //CGSize(width: 2000, height: 2000)
        canvasView.contentInset = editor.edgeInset
        ...
        if let mainMenu = EditorModel.shared.mainMenu {
            // 1. Crea un UIButton, non un UIBarButtonItem
            let customMenuButton = UIButton(type: .system)
            editor.menuButton = customMenuButton

            // 2. Imposta l'immagine e il menu direttamente sul bottone
            customMenuButton.setImage(UIImage(systemName: "command"), for: .normal)
            customMenuButton.menu = mainMenu
            
            // 3. FONDAMENTALE: Di' al bottone di mostrare il menu come sua azione principale
            customMenuButton.showsMenuAsPrimaryAction = true
            
            // 4. Ora crea il UIBarButtonItem usando il tuo bottone personalizzato come vista
            let menuButton = UIBarButtonItem(customView: customMenuButton)
        ...
        DispatchQueue.main.async{
            context.coordinator.updateNavigatorState(for: canvasView)
        }
        canvasView.onLayout = {
    ...
    func updateUIView(_ uiView: PKCanvasView, context: Context) {
        ...
        if editor.isZoomEnabled {
           ...
           uiView.minimumZoomScale = uiView.zoomScale // Blocca lo zoom al valore corrente
           uiView.maximumZoomScale = uiView.zoomScale
        }
   
        context.coordinator.rotationGestureRecognizer?.isEnabled = editor.isRotationEnabled
           
        // This existing logic will apply the offset set by the Navigator
    ...
    class Coordinator: NSObject, UIGestureRecognizerDelegate {
        ...
        init(_ parent: LayerCanvasView) {
            ...
            NotificationCenter.default.addObserver(
               self, selector: #selector(handleNavigatorUpdate),
               name: Notification.UpdateNavigator,
               object: nil
            )
        }
        
        deinit {
            NotificationCenter.default.removeObserver(self, name: Notification.UpdateNavigator, object: nil)
        }
        
        @objc func handleNavigatorUpdate() {
            if let canvasView = parent.model.canvas {
                updateNavigatorState(for: canvasView)
            }
        }
...
extension LayerCanvasView.Coordinator {
    ...
    @MainActor
    func updateNavigatorState(for canvasView: PKCanvasView) {
        let model = EditorModel.shared
        
        // 1. Calculate the visible rectangle in the drawing's coordinate space
        let visibleRect = CGRect(origin: canvasView.contentOffset, size: canvasView.bounds.size)
        let visibleRectInDrawing = CGRect(
            x: visibleRect.origin.x / canvasView.zoomScale,
            y: visibleRect.origin.y / canvasView.zoomScale,
            width: visibleRect.width / canvasView.zoomScale,
            height: visibleRect.height / canvasView.zoomScale
        )
        
        //todo check
        model.visibleRectInCanvas = visibleRectInDrawing
        
        model.navigatorImage = model.renderLayers()
    }
    ...
    func setUpGestureRecognizers(on view: UIView) {
        ...
        self.rotationGestureRecognizer = rotationGesture // <-- Assegna il gesto
    ...
    @MainActor @objc func handleRotation(_ sender: UIRotationGestureRecognizer) {
        ...
        guard parent.model.id == parent.activeCanvasId
                //,let canvasView = sender.view as? PKCanvasView
        else { return }
    ...
    @MainActor @objc func handleTap(_ sender: CanvasGestureRecognizer) {
        ...
        EditorModel.shared.selectedImageLayerID = nil
        ...
        print ("locationInDrawing \(locationInDrawing)")
        ...
        if isShapeToolSelected {
            if let tappedStroke = canvasView.drawing.strokes.last(where: { $0.renderBounds.contains(locationInDrawing) }) {
                if EditorModel.shared.selectedStroke?.randomSeed != tappedStroke.randomSeed {
                    ...
                    DispatchQueue.main.async{
                        ...
                        print("✅ Nuovo Stroke selezionato con ID: \(tappedStroke)")
                    }
            ...
            Task { @MainActor in
                ...
                EditorModel.shared.registerDrawing(
                    newDrawing,
                    on:layer.id,
...
extension LayerCanvasView.Coordinator: PKCanvasViewDelegate, PKToolPickerObserver {
    ...
    func scrollViewDidEndZooming(_ scrollView: UIScrollView, with view: UIView?, atScale scale: CGFloat) {
        ...
        if EditorModel.shared.isProgrammaticScroll { return }
    ...
    func scrollViewDidZoom(_ scrollView: UIScrollView) {
        guard parent.model.id == parent.activeCanvasId , let _ = scrollView as? PKCanvasView else { return }
        //
    ...
    // Chiamato SOLO ALLA FINE di un gesto di scorrimento
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        guard parent.model.id == parent.activeCanvasId, let canvasView = scrollView as? PKCanvasView else { return }
        // Finalizza lo stato dell'offset nel modello
        EditorModel.shared.contentOffset = canvasView.contentOffset
